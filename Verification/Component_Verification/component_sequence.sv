/*
* component_sequence generates a sequence of edgecase and randomized bits to
* send to alu.
*/
class alu_sequence extends uvm_sequence;
    ...
    function new(string name="component_sequence");
        super.new(name);
    endfunction
    ...
    virtual task body();
        for(int i=0; i < num_of_packets; i++) begin
            ...
            component_item m_component_item = component_item::type_id::create("m_component_item");

            //blocking statement for sending data to driver
            //wait till driver ready
            start_item(m_component_item); 
            m_component_item.randomize();
            `uvm_info("SEQ", $sformatf("Generate sequence item: %s", m_component_item.item_string), UVM_HIGH)
            finish_item(m_component_item);
        end
